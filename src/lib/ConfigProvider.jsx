...
function themeKeyToCssVar(key) {
  // parchmentDeep -> parchment-deep, claySoft -> clay-soft
...
function applyTheme(theme) {
  ...
  THEME_KEYS.forEach((key) => {
    ...
    if (value) root.style.setProperty(themeKeyToCssVar(key), value);
  });
}
...
export function ConfigProvider({ children }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const records = await base44.entities.CursorSiteConfig.list("-updated_date", 1);
        if (cancelled) return;
  ...
  const saveConfig = useCallback(
    async (partial) => {
      ...
      let id = recordId;
      if (id) {
        await base44.entities.CursorSiteConfig.update(id, partial);
      } else {
        const created = await base44.entities.CursorSiteConfig.create(partial);
        id = created.id;
        setRecordId(id);
      }
      setConfig(next);
...
// Full journey records (images, arc, support images from content.js) merged
// with the editable text-field overrides saved in the config.
