...
function JourneyTile({ journey, index }) {
  ...
  const depth = 22 + (index % 2) * 20;
  const y = useTransform(scrollYProgress, [0, 1], [depth, -depth]);

  return (
    <Reveal delay={index * 90}>
      <motion.div style={{ y }}>
        <Link to={`/journeys/${journey.slug}`} className="group block" ref={ref}>
          ...
          <p className="font-body text-[14px] text-ink-soft italic leading-snug mb-4">{journey.tagline}</p>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
          {journeys.map((journey, i) => (
            <JourneyTile key={journey.slug} journey={journey} index={i} />
          ))}
        </div>
      </div>
    </section>
